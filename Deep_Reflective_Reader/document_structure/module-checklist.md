...
- [ ] Define native PDF Outline two-layer normalization for scanned-image books
  Evidence needed: validated `/Outlines` with complete destinations can map to the current `chapters[].sections[]` hierarchy using PDF page indices and OCR page boundaries, including fixtures where top-level outline entries start at level `0`.
  Notes: Use the minimum outline level as the normalized chapter level; flatten all descendants under that chapter into sections in source order. Preserve original outline level, source page index, page label, normalized range, and compression reason in parse provenance only. Do not introduce `Part -> Chapter -> Section` persistence, root `sections[]`, or `structure_nodes`. Title OCR matching may refine offsets but must not be required when outline page destinations and OCR page boundaries are sufficient. Reject the projection atomically if destinations are incomplete, page order is non-monotonic, page boundaries are missing, or section ranges are empty.
